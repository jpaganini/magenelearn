...
Outputs are stored under:
  base_output_dir/
    ...
    04_model/
    05_cv/
    06_splits_evaluation/  (Train)
    07_test_evaluation/    (Test)

If --output-dir is omitted, a timestamped dir YYMMDD_HHMM is auto-created.
"""
import click
import subprocess
import sys
...
def train(output_dir, chisq, muvr, no_split,
    ...
    if not no_split:
        run_cmd(['python', '00_split_dataset.py', '--meta-file', os.path.abspath(meta_file), '--out-prefix', name], cwd=d0)
    ...
    if chisq:
        cmd = ['python','01_chisq_selection.py','--meta',os.path.abspath(train_meta),
               '--features1',os.path.abspath(features1),'--output_dir',d1]
        if features2: cmd += ['--features2', os.path.abspath(features2)]
        run_cmd(cmd)
    ...
    if muvr:
        if not chisq:
            click.echo("Error: --muvr requires --chisq", err=True); sys.exit(1)
        run_cmd(['python','02_muvr_feature_selection.py',
        ...
        files = glob.glob(pat)
        if not files:
            click.echo(f"Error: no muvr_min found in {d2}", err=True); sys.exit(1)
        muvr_file = files[0]
    else:
        muvr_file = None
    ...
    if muvr:
        run_cmd(['python','03_extract_features.py',
        ...
        click.echo("Error: pipeline requires --muvr", err=True); sys.exit(1)

    # 04: Train & 05: CV
    d4 = make_step_dir(base, 4, 'model')
    d5 = make_step_dir(base, 5, 'cv')
    run_cmd(['python','04_train_model.py',
    ...
    run_cmd(['python','05_evaluate_model.py',
